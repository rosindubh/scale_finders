# code for the ADMISSION client taken from 'making use of python'
# 1st january 2017 phil welsby

from socket import *        # Imports the attributes of the socket module
Hostname = '192.168.0.6'    # Defines the IP address of the server (IT Department computer).
#  Uses the host name/IP address of the computer on which  you are executing the server
...
Client_Socket=socket(AF_INET, SOCK_STREAM)    # Creates a stream socket for the client
Client_Socket.connect(ServerAddress)    # Connects to the server at a given address
print
print 'The client is connected to the server at', ServerAddress
print
while 1:
    DataToServer = raw_input('Enter data: ')     # Asks input for data
    if not DataToServer:     # Checks if the variable is blank
        print
        print '********************************************'
        ...
        print
        break
    Client_Socket.send(DataToServer)        # Send data to server
