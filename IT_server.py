# code for the IT dept server taken from 'making use of python'
# 31 december 2016 - phil welsby
...
Buffer = 1024        # Ddfines the maximum size of data that can be exchanged
ServerAddress = (Hostname, PortNumber) # Defines the address of the server
Server_Socket=socket(AF_INET, SOCK_STREAM)    # Creates a stream socket for the server
Server_Socket.bind(ServerAddress)        # Binds the server address to the server socker
...
while 1:        # Infinite loop starts
    Temp_Socket, ClientAddress = Server_Socket.accept()  # Accepts client connection and passes it to a new temp socket
    ...
    while 1:        # Client/server connection loop starts
        DataFromClient = Temp_Socket.recv(Buffer)        # Receives data from the client
...
Server_Socket.close()      #Closes the server socket and stops the infinite loop
